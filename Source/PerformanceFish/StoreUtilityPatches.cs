...
public class StoreUtilityPatches : ClassWithFishPatches
{
	public class IsInValidBestStorage_Patch : FishPatch
	{
		public override string Description { get; }
			= "Patches IsInValidBestStorage to immediately remove things from haulables when they're found to be in "
				+ "the best storage. Default often has them checked several more times before removal happens";

		public override Delegate TargetMethodGroup { get; } = StoreUtility.IsInValidBestStorage;
		...
		public static void Postfix(Thing t, bool __result)
		{
			...
			{
				if (!StoreUtility.TryFindBestBetterStorageFor(t, null, t.Map, StoragePriority.Unstored,
					Faction.OfPlayerSilentFail, out IntVec3 _, out IHaulDestination _, needAccurateResult: false))
				{
	...
	public class IsGoodStoreCell_Patch : FishPatch
	{
		...
		public override Delegate TargetMethodGroup { get; } = StoreUtility.IsGoodStoreCell;
	...
	public class GetStatValue_Patch : FishPatch
	{
		...
		public override Delegate TargetMethodGroup { get; } = StatExtension.GetStatValue;
